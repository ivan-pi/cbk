...
* **`work`**: the `min(m, n)` scalars `tau` per matrix, in compact format.
  `(ap, work)` is the `(H, tau)` that `cqr_mkl_?ormqr_compact` accepts, so
...
the toolkit's (README, "Threading"): static-schedule OpenMP, at most one
...
portable `?gels_compact` of `cqr_compact.h` is the checked surface.
...
* **Suite 1, vs dense `LAPACKE_?gels`**: every `(layout, trans)` over square,
  ...
  random `B`, solved by `cqr_mkl_?gels_compact` and per matrix by
...
* **Suite 2, vs the three-step pipeline**: on the same packed square batch
  with `B = A X` for a known `X`, `cqr_mkl_?gels_compact` and
  `mkl_?geqrf_compact -> cqr_mkl_?ormqr_compact -> mkl_?trsm_compact` must
...
* **MKL-style API** (`cqr_mkl_ext.h`): `cqr_mkl_?gels_compact`, unwrapping
  `MKL_COMPACT_PACK -> V`, instantiated on `MKL_INT`; no checking; `lwork = -1`
  answers the query.
* **Portable C API** (`cqr_compact.h`): `?gels_compact` with an explicit `V`,
...
* **Kernel** (`src/cqr_gels_compact.hpp`): `gels_compact_group<T,V>` over the
...
interoperate with `cqr_mkl_?ormqr_compact` and `cqr_mkl_?trsm_compact` for
