...
factorization, `ormqr_compact_group` (the backward sweep) for `Q B`, and
`trsm_compact_group` for `R X = B` / `R^T Y = B`. That group kernel routes on
the views: with `B`'s rows contiguous it reaches the tuned `trsm_left_dot`
whether `F` is column-major (`m >= n`) or a column-major matrix seen through
the transposed view (`m < n`, where `R` is the lower triangle `L = R^T` of `A`'s
own storage and the solve flips `uplo` and `transa`); row-major goes through
the strided kernel. `?gels` itself also scales `A` and `B` when their norms are
near overflow or underflow; that rescaling is out of scope here, as it is for
every routine in the toolkit (section 6.7).

### 6.2 The fused `[F | B]` reduction

In the overdetermined case `Q^T B` is not a separate sweep.
`geqrf_panel_compact_group`, a sibling of `geqrf_compact_group` over the same
one-reflector step (`geqrf_reflector`), takes a panel: every reflector
`H(kk)`, right after it is built from column `kk` of `F`, is applied to the
trailing columns of `F` *and* to all columns of `B` -- the QR factorization of
the fused matrix `[F | B]`, truncated to `F`'s `q` reflectors. It is a separate
kernel rather than a flag on `geqrf_compact_group` because an extra parameter,
even one compiled out, changed GCC's code generation for the plain path (a
different frame and register allocation, measured at 8-13%); with the plain
signature untouched, its assembly matches the pre-`gels` kernel's. This is exactly what `ormqr('L','T')` computes (the
...
triangular solve is `trsm`'s 4/2/1-blocked row-dot, for `m >= n` and, through
the transposed view, for `m < n` (section 6.1). Row-major runs the same code
through strided views (correctness-first, as for `geqrf`, `ormqr` and `trsm`).
...
* **Portable C API** (`cqr_compact.h`): `dgels_compact` / `sgels_compact`, with
  ...
  `info = -j` validation. Like the rest of that surface it drops LAPACK's
  workspace protocol: the `tau` buffer is an explicit `taup` output, sized
  exactly as `?geqrf_compact`'s, instead of `work`/`lwork` and a query.
* **Templated kernel** (`src/cqr_gels_compact.hpp`): the per-group
  ...
  factorization is `geqrf_compact_group` (`geqrf_panel_compact_group` with the
  fused panel), the apply-`Q`
  is `ormqr_compact_group`, the triangular step `trsm`'s group kernels;
  `gels_lwork` is the workspace rule of the MKL-style adapter and its tests.
