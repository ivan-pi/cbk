# API Design Document: `cbk_dorgqr_compact`
...
the compact form of LAPACK `?orgqr`, supplied by the `cbk` project on the
precedent of `cbk_?ormqr_compact`. It completes the standard QR trio
...
`cbk_?gels_compact`) left in `ap` and `taup` -- in place: on exit the
...
pack width (the `cbk_?ormqr_compact` design records the same
...
Step 1 is exactly `larf` (`cbk_ormqr_compact.hpp`), the register-blocked
...
header `src/cbk_orgqr_compact.hpp`; nothing is added to any existing
...
the portable `?orgqr_compact` of `cbk.h` is the checked surface.
...
e.g. the `m < n` output of `cbk_?gels_compact` -- is this kernel over
...
* **Suite 1, vs dense `LAPACKE_?orgqr`** (the dispatch already exists in
  ...
  difference -- then `Q` formed both ways: `cbk_?orgqr_compact` on the
...
* **Suite 2, vs `cbk_?ormqr_compact` on a packed identity**: the two
...
* **Portable C API** (`cbk.h`):
  ...
  `nm < 0`.
* **Kernel** (`src/cbk_orgqr_compact.hpp`): `orgqr_compact_group<T,V>`
