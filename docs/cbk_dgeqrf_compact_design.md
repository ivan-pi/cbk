# API Design Document: `cbk_dgeqrf_compact`
...
The routine produces the reflectors `(H, tau)` that `cbk_?ormqr_compact`
...
real precisions are provided: `cbk_dgeqrf_compact` (double) and
`cbk_sgeqrf_compact` (single).
...
* **`work`, `lwork`** (`double *`, `MKL_INT`): workspace. This unblocked kernel
  ...
  (see the workspace note in `cbk_compat.h`).
...
`cbk_?geqrf_compact` validates nothing and writes a single scalar
...
`cbk_dgeqrf_compact -> cbk_dormqr_compact('L','T') -> mkl_dtrsm_compact`
...
`cbk::detail::pack<T,V>` GNU-vector machinery.

### 8.1 API boundary

* **MKL-style API** (`cbk_compat.h`, the primary surface):
  `cbk_dgeqrf_compact` / `cbk_sgeqrf_compact`, unwrapping
  `MKL_COMPACT_PACK -> V` and instantiated on `MKL_INT` so ILP64 dimensions are
  not narrowed.
* **Portable C API** (`cbk.h`): `dgeqrf_compact` / `sgeqrf_compact`
  ...
  validation.
* **Templated kernel** (`src/cbk_geqrf_compact.hpp`): the per-group kernel
  ...
  (`src/cbk.cpp`, `src/cbk_compat.cpp`) call. The trailing-column
