# API Design Document: `cbk_dpotrf_compact` / `cbk_dpotrs_compact` / `cbk_dposv_compact`
...
no compact `potrs` or `posv`, so -- like `cbk_?ormqr_compact` -- the solve
and the fused driver fill a gap in the compact ecosystem rather than shadowing
an MKL routine.

They join the QR routines already in this project (`cbk_?geqrf_compact`,
`cbk_?ormqr_compact`), sharing the same `pack<T,V>` GNU-vector machinery and
...
```
cbk_dpotrf_compact('L', A -> L);          // A = L L^T
cbk_dpotrs_compact('L', L, B);           // B := A^{-1} B = X
                                             //   (L z = B; L^T X = z)

cbk_dposv_compact ('L', A -> L, B);      // both, fused per group
```

(`cbk_dpotrs_compact` packages the two `mkl_dtrsm_compact('L','L','N')` /
...
relation `cbk_?sytrsnp_compact` / `cbk_?sysvnp_compact` bear to their
...
`cbk_?potrf_compact` forms the Cholesky factorization of each symmetric positive-definite
...
compact routine (e.g. `cbk_?potrs_compact` or `mkl_?trsm_compact`) will
consume the factors first.

`cbk_?potrs_compact` then solves `A X = B` for each matrix from that
...
`cbk_?posv_compact` performs the factorization and the solve for each group
of `V` matrices before moving to the next group (section 6.8). On exit `ap`
holds the factor exactly as `cbk_?potrf_compact` leaves it and `bp` holds
...
Factorization (`cbk_?potrf_compact`):
...
Solve (`cbk_?potrs_compact`), in addition:

* **`nrhs`** (`MKL_INT`): number of right-hand sides (columns of `B`,
  `nrhs >= 0`).
* **`ap`** (`const double *`): the factored compact batch from
  `cbk_?potrf_compact` (not modified); `uplo` must match the factorization
  call.
...
Fused solve (`cbk_?posv_compact`): the solve's arguments, with **`ap`**
...
The batch is factored with the right-looking LAPACK algorithm (`dpotf2`),
...
lifts almost verbatim with `double -> V`-wide vector: every `+`, `-`, `*`, `/`,
and `sqrt` becomes a lane-wise SIMD operation over `V` independent matrices.
For the lower triangle the sweep is, per pivot column `j`:
...
Within a panel the pivot's operations are reordered for latency. The plain
sweep's chain from one pivot to the next is `sqrt`, divide, scale, update --
two long-latency operations on the one divider port, some 50 cycles per column
on AVX-512, which at `n <= 16` *is* the running time (the FMA work is a tenth
of it). The kernel instead runs each pivot's rank-1 update on the
still-unscaled column with the weights scaled by `1/a`,
...
so the next pivot waits on one divide, a multiply and an FMA; the square root
and the column scaling follow off that chain, and `1/sqrt(a)` costs a multiply
rather than a second divide. This is the LDL^T update of the same SPD matrix
(`d = a`, `l = A(:,j)/a`, exactly `cbk_?sytrfnp_compact`'s step -- the panel
...
`cbk_?potrf_compact` validates no arguments and writes a single scalar
...
`cbk_?potrs_compact` completes the batched solve from the factor. Its two
...
`cbk_?sytrsnp_compact` (whose diagonal-solve middle step Cholesky does not
...
`cbk_?trsm_compact` (or `mkl_?trsm_compact`) twice, as in section 1 --
...
`docs/cbk_dsytrfnp_compact_design.md` section 6.8. `bench_posv_compact`
...
* **Elementwise vs LAPACK (gated).** Unlike QR reflectors, the SPD Cholesky
  ...
  printed: `|| L_cbk - L_lapack ||_1 / ||L_lapack||_1 <= 20 * n * eps`.

### 7.2 Suite 2 -- Cross-check vs `mkl_dpotrf_compact`

The same packed batch is factored by both `cbk_dpotrf_compact` and the native
...
`cbk_dpotrf_compact('L') -> mkl_dtrsm_compact('L','L','N') ->
...
`cbk_?potrf_compact -> cbk_?potrs_compact` must recover `X` (forward
...
blocks. On the same packed input `cbk_?posv_compact` must reproduce the
...
`cbk::detail::pack<T,V>` GNU-vector machinery and a lane-wise `vsqrt<T,V>` helper.

### 8.1 API boundary

* **MKL-style API** (`cbk_compat.h`, the primary surface):
  `cbk_?potrf_compact` / `cbk_?potrs_compact` / `cbk_?posv_compact`,
  unwrapping `MKL_COMPACT_PACK -> V` and `MKL_UPLO`/`MKL_LAYOUT`, instantiated
  on `MKL_INT` so ILP64 dimensions are not narrowed.
* **Portable C API** (`cbk.h`): `?potrf_compact` / `?potrs_compact` /
...
| File | Role |
|------|------|
| `src/cbk_potrf_compact.hpp` | Templated SIMD Cholesky kernel (vectorized `potf2`; scalar `T`, width `V`). |
| `src/cbk_potrs_compact.hpp` | Templated solve: two non-unit `trsm` group sweeps, and its driver. |
