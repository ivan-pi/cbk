# API Design Document: `cbk_dsytrfnp_compact` / `cbk_dsytrsnp_compact` / `cbk_dsysvnp_compact`
...
itself ships no compact `sytrf` of any kind, so -- like `cbk_?ormqr_compact`
...
The factorization is the square-root-free sibling of `cbk_?potrf_compact`:
...
```
cbk_dsytrfnp_compact('L', A -> (L, D));    // A = L D L^T
cbk_dsytrsnp_compact('L', A, B);           // B := A^{-1} B = X
                                               //   (L z = B; D w = z; L^T X = w)

cbk_dsysvnp_compact ('L', A -> (L, D), B); // both, fused per group
```
...
`cbk_?sytrfnp_compact` forms, for each symmetric `n x n` matrix `A` in the
...
`cbk_?sytrsnp_compact` then solves `A X = B` for each matrix from that
...
`cbk_?sysvnp_compact` performs both steps for each group of `V` matrices
before moving to the next group (section 6.8). On exit `ap` holds the factor
exactly as `cbk_?sytrfnp_compact` leaves it and `bp` holds `X`; the result
...
factor straight to `cbk_?sytrsnp_compact`, which consumes it in place.

## 4. Input Parameters

Factorization (`cbk_?sytrfnp_compact`):
...
Fused solve (`cbk_?sysvnp_compact`): the solve's arguments, with **`ap`**
...
Following the Compact convention (and `cbk_?potrf_compact`'s treatment of
...
`cbk_?sytrsnp_compact` completes the batched solve from the factor. Its two
...
`cbk_?trsm_compact(diag = MKL_UNIT)` twice and scale the middle step --
...
`cbk_?sysvnp_compact` is the `?sysv` of the pair, and it exists for
...
every factor is written out and read back. The fused driver runs, for each
...
factored by `cbk_?sytrfnp_compact` (col-major, lower) and by MKL's
...
`B = A X` for known `X` on indefinite batches, then `cbk_?sytrfnp_compact
-> cbk_?sytrsnp_compact` must recover `X`, over both `uplo`, both layouts,
...
`cbk_?sysvnp_compact` must reproduce the two-step factor and `X`
...
`cbk::detail::pack<T,V>` machinery, the `BatchView` addressing, the
...
* **MKL-style API** (`cbk_compat.h`, the primary surface):
  `cbk_?sytrfnp_compact` / `cbk_?sytrsnp_compact` /
  `cbk_?sysvnp_compact`, unwrapping `MKL_COMPACT_PACK -> V` and
  `MKL_UPLO`/`MKL_LAYOUT`, instantiated on `MKL_INT` so ILP64 dimensions are
  not narrowed.
* **Portable C API** (`cbk.h`): `?sytrfnp_compact` / `?sytrsnp_compact`
...
`compact<T>` / `compat<T>` / `mkl<T>` dispatch entries live in
