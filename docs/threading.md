...
compromise between two measured break-evens (the `CQR_OMP_MIN_FLOPS`
preprocessor macro overrides it, e.g.
`-DCMAKE_CXX_FLAGS="-DCQR_OMP_MIN_FLOPS=1e5"`). Because the thread count is
...
`for_each_group<V>(nm, flops_per_group, body)` in `src/cqr_compact_common.hpp`;
the gate described above lives there, and no kernel threads inside a group.
`-DCQR_WITH_OPENMP=OFF` compiles the routines single-threaded.
