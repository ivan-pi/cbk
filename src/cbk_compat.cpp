/* cbk_compat.cpp
 *
 * The MKL-style API (cbk_compat.h): thin C-linkage adapters that unwrap the
...
#include "cbk_orgqr_compact.hpp"
#include "cbk_potrf_compact.hpp"
#include "cbk_potrs_compact.hpp"
#include "cbk_posv_compact.hpp"
#include "cbk_sytrfnp_compact.hpp"
#include "cbk_sytrsnp_compact.hpp"
#include "cbk_sysvnp_compact.hpp"
#include "cbk_trsm_compact.hpp"
#include "cbk_gels_compact.hpp"
           ...
           MKL_INT lwork, MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)
{
    ...
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::geqrf_compact<T, decltype(v)::value, MKL_INT>(rowmajor, m, n, ap,
                                                                   ldap, taup, nm);
    });
}
           ...
           MKL_INT lwork, MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)
{
    ...
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::ormqr_compact<T, decltype(v)::value, MKL_INT>(
           ...
           MKL_INT nm)
{
    *info = 0;
    ...
    const bool rowmajor = (layout == MKL_ROW_MAJOR);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::orgqr_compact<T, decltype(v)::value, MKL_INT>(rowmajor, m, n, k, ap,
                                                                   ldap, taup, nm);
    });
}
           ...
           MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)
{
    ...
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::potrf_compact<T, decltype(v)::value, MKL_INT>(rowmajor, upper, n, ap,
                                                                   ldap, nm);
    });
}
           ...
           MKL_INT nm)
{
    *info = 0;
    ...
    const bool upper = (uplo == MKL_UPPER);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::potrs_compact<T, decltype(v)::value, MKL_INT>(
          ...
          T *bp, MKL_INT ldbp, MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)
{
    *info = 0;
    ...
    const bool upper = (uplo == MKL_UPPER);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::posv_compact<T, decltype(v)::value, MKL_INT>(
            rowmajor, upper, n, nrhs, ap, ldap, bp, ldbp, nm);
    });
}
             ...
             MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)
{
    *info = 0;
    ...
    const bool upper = (uplo == MKL_UPPER);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::sytrfnp_compact<T, decltype(v)::value, MKL_INT>(rowmajor, upper, n,
                                                                     ap, ldap, nm);
    });
}
             ...
             MKL_INT nm)
{
    *info = 0;
    ...
    const bool upper = (uplo == MKL_UPPER);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::sytrsnp_compact<T, decltype(v)::value, MKL_INT>(
            rowmajor, upper, n, nrhs, ap, ldap, bp, ldbp, nm);
    });
}
            ...
            MKL_INT nm)
{
    *info = 0;
    ...
    const bool upper = (uplo == MKL_UPPER);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::sysvnp_compact<T, decltype(v)::value, MKL_INT>(
            rowmajor, upper, n, nrhs, ap, ldap, bp, ldbp, nm);
    });
}
          ...
          MKL_INT ldbp, MKL_COMPACT_PACK format, MKL_INT nm)
{
    ...
    run_format<T>(format, [&](auto v) {
        cbk::detail::trsm_compact<T, decltype(v)::value, MKL_INT>(
          ...
          MKL_COMPACT_PACK format, MKL_INT nm)
{
    if (lwork == -1) { /* workspace query: the tau scratch, one slot per group */
        ...
        if (V) work[0] = T(cbk::detail::gels_lwork(m, n, nm, V));
        *info = V ? 0 : -1;
        return;
    }
    *info = 0;
    ...
    const bool rowmajor = (layout == MKL_ROW_MAJOR);
    *info = run_format<T>(format, [&](auto v) {
        cbk::detail::gels_compact<T, decltype(v)::value, MKL_INT>(
...
#define CBK_DEFINE_COMPAT_ENTRY_POINTS(T, p)                                             \
    void cbk_##p##geqrf_compact(MKL_LAYOUT layout, MKL_INT m, MKL_INT n, T *ap,          \
                                MKL_INT ldap, T *taup, T *work, MKL_INT lwork,           \
                                MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)      \
    {                                                                                    \
        geqrf(layout, m, n, ap, ldap, taup, work, lwork, info, format, nm);              \
    }                                                                                    \
    void cbk_##p##ormqr_compact(                                                         \
    ...
    void cbk_##p##orgqr_compact(MKL_LAYOUT layout, MKL_INT m, MKL_INT n, MKL_INT k,      \
    ...
    void cbk_##p##potrf_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n, T *ap,      \
                                MKL_INT ldap, MKL_INT *info, MKL_COMPACT_PACK format,    \
                                MKL_INT nm)                                              \
    {                                                                                    \
        potrf(layout, uplo, n, ap, ldap, info, format, nm);                              \
    }                                                                                    \
    void cbk_##p##potrs_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n,             \
                                MKL_INT nrhs, const T *ap, MKL_INT ldap, T *bp,          \
                                MKL_INT ldbp, MKL_INT *info, MKL_COMPACT_PACK format,    \
                                MKL_INT nm)                                              \
    {                                                                                    \
        potrs(layout, uplo, n, nrhs, ap, ldap, bp, ldbp, info, format, nm);              \
    }                                                                                    \
    void cbk_##p##posv_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n,              \
                               MKL_INT nrhs, T *ap, MKL_INT ldap, T *bp, MKL_INT ldbp,   \
                               MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)       \
    {                                                                                    \
        posv(layout, uplo, n, nrhs, ap, ldap, bp, ldbp, info, format, nm);               \
    }                                                                                    \
    void cbk_##p##sytrfnp_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n, T *ap,    \
                                  MKL_INT ldap, MKL_INT *info, MKL_COMPACT_PACK format,  \
                                  MKL_INT nm)                                            \
    {                                                                                    \
        sytrfnp(layout, uplo, n, ap, ldap, info, format, nm);                            \
    }                                                                                    \
    void cbk_##p##sytrsnp_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n,           \
                                  MKL_INT nrhs, const T *ap, MKL_INT ldap, T *bp,        \
                                  MKL_INT ldbp, MKL_INT *info, MKL_COMPACT_PACK format,  \
                                  MKL_INT nm)                                            \
    {                                                                                    \
    ...
    void cbk_##p##sysvnp_compact(MKL_LAYOUT layout, MKL_UPLO uplo, MKL_INT n,            \
                                 MKL_INT nrhs, T *ap, MKL_INT ldap, T *bp, MKL_INT ldbp, \
                                 MKL_INT *info, MKL_COMPACT_PACK format, MKL_INT nm)     \
    {                                                                                    \
        sysvnp(layout, uplo, n, nrhs, ap, ldap, bp, ldbp, info, format, nm);             \
    }                                                                                    \
    void cbk_##p##trsm_compact(MKL_LAYOUT layout, MKL_SIDE side, MKL_UPLO uplo,          \
    ...
    void cbk_##p##gels_compact(MKL_LAYOUT layout, char trans, MKL_INT m, MKL_INT n,      \
                               MKL_INT nrhs, T *ap, MKL_INT ldap, T *bp, MKL_INT ldbp,   \
...
extern "C" {
CBK_DEFINE_COMPAT_ENTRY_POINTS(double, d)
CBK_DEFINE_COMPAT_ENTRY_POINTS(float, s)
} /* extern "C" */

#undef CBK_DEFINE_COMPAT_ENTRY_POINTS
