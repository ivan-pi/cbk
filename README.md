...
compromise between two measured break-evens (`-DCQR_OMP_MIN_FLOPS=...`
overrides it). Because the thread count is OpenMP's team size *at the current
nesting level*, the routines compose with a caller's own parallel loop: called
...
chosen). The portable interfaces declare their dummies in the compact shape
-- matrices `ap(v, ldap, ncols, *)` with the group count assumed (pad `nm`
up to a multiple of `V`), tau `taup(v, min(m, n), *)` -- so naturally shaped
arrays go through those generics as they are. The MKL-style dummies stay
rank-1 assumed-size (`V` travels in the `format` enumerator, not as a
dummy), so those generics take a flat buffer or a rank-1 pointer view,
`p(1:size(a)) => a`. The specific names of both APIs accept an array of any
rank, or a starting array element, by sequence association. Compile fixed-form includers at the standard 72-column line
length.
...
| `src/cqr_compact.cpp` | The portable C API: argument validation and `V` dispatch for all sixteen entry points. |
| `src/cqr_mkl_ext.cpp` | The MKL-style API: MKL enum / `MKL_COMPACT_PACK` unwrapping for all sixteen entry points. |
