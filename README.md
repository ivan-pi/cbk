...
* **`cqr_mkl_?orgqr_compact`** - the *form-Q* counterpart: LAPACK `?orgqr` for
  the compact format, generating the explicit (thin) `Q` in place over the
  reflectors a compact QR left behind -- for the batches that need `Q` as a
