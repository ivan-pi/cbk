...
// (L, D) must reconstruct A. Lower: W = L D (columns of the unit-lower L
// scaled by D), then W := W L^T by ?trmm (side 'R', unit diagonal). Upper:
// W = D U (rows scaled), then W := U^T W (side 'L'). Gated at
...
template <class T> static int test_reference(int n, char uplo)
{
    ...
    for (int t = 0; t < trials; ++t) {
        ...
        copy_matrix(A, F);
        ref_sytf2np(uplo, F);
        // W: the unit factor with D folded in, the other triangle zeroed
        for (int j = 0; j < n; ++j)
            for (int i = 0; i < n; ++i) {
                if (i == j)
                    W(i, j) = F(i, i);
                else if (upper)
                    W(i, j) = (i < j) ? F(i, i) * F(i, j) : T(0); // D U
                else
                    W(i, j) = (i > j) ? F(i, j) * F(j, j) : T(0); // L D
            }
        if (upper)
            tri_apply('L', 'U', 'T', 'U', F, W, R); // R = U^T (D U)
        else
            tri_apply('R', 'L', 'T', 'U', F, W, R); // R = (L D) L^T
        worst = std::max(worst, max_abs_diff(Rs.data(), As.data(), As.size()) /
                                    std::max(norm1(A), norm_floor));
    }
