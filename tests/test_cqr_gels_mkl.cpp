 ...
 *   solved by cqr_mkl_?gels_compact (packed in `layout`) and per matrix by
...
template <class T> int suite2(int nm, int n, int nrhs)
{
    ...
    T wq;
    cqr_mkl<T>::gels(MKL_COL_MAJOR, 'N', n, n, nrhs, ap1.get(), n, bp1.get(), n, &wq, -1,
                     &info, fmt, nm);
    std::vector<T> work((size_t)std::max<MKL_INT>((MKL_INT)wq, 1));
    cqr_mkl<T>::gels(MKL_COL_MAJOR, 'N', n, n, nrhs, ap1.get(), n, bp1.get(), n,
    ...
    T dummy;
    cqr_mkl<T>::ormqr(MKL_COL_MAJOR, 'L', 'T', n, nrhs, n, ap2.get(), n, tp2.get(),
